...
Both files must be UTF-8 CSV without a byte order mark, with exactly the ordered header `request_id,duration_ms,status`, at least one row, unique nonempty request identifiers without leading or trailing whitespace within each file, durations in milliseconds written as nonnegative decimal numbers (digits with an optional fractional part, such as `100` or `12.5`), and `status` equal to `ok` or `error`. The tool reports whether both files contain the same set of request identifiers; whether those identifiers represent the same fixed request set remains a judgment from the measurement context. Malformed or incomplete rows are rejected. The minimum sample count must be a positive integer, the other limits must be written in the same decimal form as durations, and error proportion must lie between zero and one.
