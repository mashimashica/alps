...
Design the configuration and interaction of agents, tools, information resources, and execution environments suited to the target work and its applicable conditions, and assess its feasibility and effectiveness with evidence proportionate to the request. Defining what the work must achieve is outside this Skill; it starts from a description of the work.
...
2. For a target Skill, reflect the work, judgment criteria, required tool use, and conditions in `SKILL.md` and necessary resources, preserving the meaning of the target work description. Put bundled processing required by the target work in that Skill's `scripts/` and document it using the script guidance; scripts in this design Skill must support its design or evaluation work.
