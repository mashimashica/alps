...
この資料は、読みやすいSkill Descriptionを起草するための参考例であり、発見層と実行層の物理的な分離を要求しない。特定の見出し、並び順、箇条番号または識別子を要求しない。Process Framework、ALPSおよび適用される実行環境の規則を優先する。
...
<本Skillの存在理由となる、相互に関連する一つまたは複数の上位目的を簡潔に記述する。>
<必要な場合は、Purposeの意味を変えない範囲の説明、対象領域または厳密さの考え方を、参考情報として続ける。>

## Outcomes

- a) <肯定的で観察可能かつ評価可能な一つの結果状態が成立している。>
- b) <一つの独立した結果が成立している。>
...
- <Outcome、Activity、Taskおよび証拠の対応を識別するのが望ましい。>
- <Outputと受領側のInputの名称、意味および適用範囲を整合させるのが望ましい。>

## Bundled Resources

- [<参照資料>](references/<reference>.md): <役割と参照する条件。>
- `scripts/<script>.*`: <役割、使用条件および限界。>
- `assets/<asset>.*`: <発見、提示またはOutputの作成における役割。>
...
- Constraint、判定基準または品質条件をTaskに含める場合、それらを行為の対象または行為を方向付ける条件として扱い、Taskの主たる機能を個別の行為として維持する。
- 主たる機能が個別の行為ではない記述は、その機能に対応する別の要素に置く。
- Taskごとに、要求事項、禁止事項、推奨事項、許容される行為または通常実施される行為のいずれかを規範語で判別可能にする。すべての種類を含める必要はない。
- Agent、ツールまたは実行環境はInputに含めず、Enablerとして扱う。
- 必須の参照先は、対象環境から特定し、取得できる必要がある。Packageには、Skillの理解・実行またはOutputの作成を直接支援する付随資源だけを含めるのが望ましい。
