---
name: apply-alps
description: Select Agent Skills that provide authoritative descriptions of Processes suited to an application situation's needs, conditions, and risks; assess Entry/Exit Criteria and applicable Control, Constraint, and Tailoring decisions; apply the Processes singly or in combination; and manage Output/Input handoffs and the completeness and consistency of the composition as a whole. Use when work applies existing Processes through Skills, composes multiple Processes, or requires evidence of the application rationale or Outcome achievement. Do not use when the work only defines a new Skill or only adopts, changes, or retires Skill assets. ALPS-conformant.
---
...
The order in which the headings, Activities, Tasks, and numbers below appear does not prescribe a procedure or execution sequence. This Process's Entry Criteria are the decision conditions before invocation, and its Exit Criteria are the decision conditions before declaring completion; Controls and Constraints apply regardless of where they appear. The Entry Criteria of each selected Process are assessed separately before invoking that Process through its providing Skill. Use Iteration, Concurrency, Recursion, and Integration as appropriate to the application situation.

### Skill Selection

This Activity determines the Processes to apply, the Skills providing their authoritative descriptions, and the form of application. It contributes primarily to a), b), and c).

1. The needs and conditions of the application situation and the applicable Constraints must be identified.
2. Typically, the needs are compared with the Purposes and Outcomes of the Processes described by candidate Skills.
...
This Activity uses the selected Skills to execute Instances of the represented Processes and achieve the Outcomes declared in their Process Descriptions. It contributes primarily to c), d), and e).

1. Whether the described Process's Entry Criteria are satisfied must be assessed before the Process is invoked through its providing Skill. If they are not satisfied, invocation must be deferred or the unmet conditions must be resolved first.
...
12. Lessons learned from execution may be handed off to Assessment and Improvement in the management Process.

### Process Orchestration

This Activity combines multiple Processes and manages their interfaces, handoffs, and the completeness and consistency of the composition as a whole. It contributes primarily to e), f), and g).

1. The target set of Outcomes must be identified.
2. The provenance of each Process and the Skill providing its authoritative description should be identified.
...
9. When the same information item is changed by multiple Processes, how its integrity, state, and changes are handled must be defined in proportion to the quality risk.
...
Typical Outputs include the decision on which Processes to apply and the Skills providing their descriptions, the form of application, Outputs defined by Skill Descriptions, the definition of the Process composition, Outputs of the composition as a whole, and execution and decision records. These do not prescribe the only method of execution.
...
These Entry Criteria are the conditions for starting this application Process. The Entry Criteria of each selected Process must be assessed separately before invocation through its providing Skill.

## Exit Criteria

- The needs, conditions, selection result, form of application, and risk decision have been recorded.
- The Entry/Exit Criteria, requirement Tasks, Outcome evidence, and unresolved matters for each selected Process have been assessed.
...
- Before executing a Process, the parts of its Skill Description needed for execution must be reviewed. Information used only to identify candidates must not be treated as sufficient for execution.
- A Process must not be invoked through its providing Skill before its Entry Criteria are satisfied. Completion must not be declared before its Exit Criteria have been assessed.
...
- When a decision that no Process will be applied makes some Outcomes inapplicable, those Outcomes must be declared, Full Conformance must not be claimed, and Tailored Conformance under ALPS 12.3 must be used.
- When a change or exclusion causes the selected Full Conformance basis not to be met, a claim of Tailored Conformance must declare the Process tailored through the management Process and its scope and must demonstrate satisfaction of every Outcome and Activity/Task requirement that remains within that scope.
...
|---|---|---|---|
| Management Process | Managed Skill information, Tailoring decisions, application conditions | Application Process | Recipient Activities: Skill Selection and Process Execution. Asset, version, status, scope, and conditions. |
| Process A | Declared Output | Process B | Recipient Input: declared Input of Process B. Name, meaning, scope, quality conditions, and state. |
| This application Process | Execution records, decisions, lessons learned, measurement results | Management Process | Recipient Activity: Assessment and Improvement. Process Instances, evidence, limitations, and proposed changes. |
...
When a change to an Output affects another Process's Input, the affected Process and Input should be identified and the necessary reevaluation should be performed.
...
- For combined application, a table of the provider Process, Output, receiver Process, Input, meaning, scope, and quality conditions can make handoffs easier to verify.
