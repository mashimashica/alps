# Minimal Process Description template

```markdown
---
name: <lowercase-hyphen-name>
description: <What this Skill does and when to use it.>
---

# <Process Name>

## Purpose

<Why the work is undertaken.>

## Outcomes

- <An observable result condition. Add further Outcomes only as needed for the Purpose.>
```

The frontmatter is ordinary Agent Skill discovery information. Name, Purpose, and one or more Outcomes are the required Process content. Replace the placeholders; do not add empty optional sections. No fixed artifact, repeat-use assumption, Context, ID, or independent version field is required.

Each Outcome describes a result state, and the set must be relevant and sufficient for the Purpose. Use [examples](examples.md) when additional work detail, conditions, references, or evaluation context is needed. This template is informative; the [Framework](../../../spec/process-framework.md) and [Specification](../../../spec/ALPS-SPEC.md) govern meaning and representation.

[Japanese translation](locales/ja/SKILL-template.md)
