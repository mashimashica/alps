...
AIエージェントに仕事を任せるとき、すべてをモデルの判断に委ねる必要はありません。仕事には、確立した処理として実装できる部分と、状況に応じた解釈・選択・構成が必要な部分があります。
...
この図では、**Why / What**が仕事の意味、**How**が利用可能な手段、その**間の判断をAgentが担います**。エージェントは状況を解釈し、能力を選び、組み合わせ、方法を適応させます。既存の手段で足りなければ新しい手段を考案できます。一方、確立した計算、変換、検査、安定した手順は、適切な実装が担えるならツールへ委ねます。
