...
多くの場合は、最初に`alps-reference-model`を読み込み、適用する参照プロセスの選択に用います。モデルの読込み自体はプロセスの呼び出しではありません。
...
| 表現 | 役割 | 読込みによる効果 |
| --- | --- | --- |
| プロセス | 目的と成果によって独立した作業を定義します。 | プロセスを呼び出せます。 |
| プロセスモデル | 関係するプロセスとその関係を構成します。 | モデルを読み込みます。読み込むだけではプロセスを呼び出しません。 |
| プロセス参照モデル | プロセスを名称、目的、成果で定義し、明示的に関係付けます。 | モデルを読み込みます。読み込むだけではプロセスを呼び出しません。 |
| プロセスビュー | 特定の関心事または目的について複数プロセスを横断して活動とタスクを構成し、その適用方法を説明します。 | ビューを読み込みます。読み込むだけではプロセスを呼び出しません。 |
