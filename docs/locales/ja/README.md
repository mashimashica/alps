...
ALPSは、繰り返し行うエージェント作業を、再利用可能でアセスメント可能なプロセススキルへ変えます。ワークフローを一度限りのプロンプトや文書化されていない慣行のままにせず、目的と成果を出発点とし、ニーズまたはリスクによって正当化される場合だけ、境界、作業詳細および証拠を追加します。
...
| --- | --- |
| 繰り返し使うプロンプトは動作しても、その意図と完了の定義は人の頭の中にあります。 | 目的と成果を再利用可能な最小要素とし、必要な場合に境界および基準を追加します。 |
...
| 意図 | 使用するスキル |
| --- | --- |
| ALPS表現を作成、再定義または検証する | [`define-alps`](../../../skills/define-alps/references/locales/ja/SKILL.md) |
| 表現を利用し、プロセスを選択・呼び出し、または複数プロセスを組み合わせる | [`apply-alps`](../../../skills/apply-alps/references/locales/ja/SKILL.md) |
| 表現を採用、テーラリング、アセスメント、変更、改善または廃止する | [`manage-alps`](../../../skills/manage-alps/references/locales/ja/SKILL.md) |
...
```text
この反復的なリリースチェックリストを、再利用可能なプロセススキルにしてください。目的と成果から始め、必要な箇所だけ境界、入力、出力および検証証拠を追加してください。
...
[プロセスフレームワーク](../../../spec/process-framework.md)は、これらの概念と意味を定義します。プロセス記述には名称、目的および成果が必要であり、他の要素はその記述に必要な場合だけ追加します。活動とタスクは作業内容であり、実装手順ではありません。入力は出力へ変換される項目です。人、エージェント、ツールおよび実行環境は、入力ではなく資源または実行支援要素です。
...
[`alps-reference-model`](../../../skills/alps-reference-model/references/locales/ja/SKILL.md)スキルは、三つの運用プロセスを理解し選択するための補助マップです。四番目の操作ではなく、プロセスとして呼び出すものでもありません。三つのプロセスは固定段階ではなく、必要に応じて並行的、反復的または再帰的に適用してよいものです。
...
ALPSはリポジトリ全体を一つのリリース単位として版管理します。現在の版は**0.5.0**であり、初期開発段階にあります。正確なリリース内容はGitタグとコミットで特定します。
