...
- Clarify Framework wording: state that Outcomes are the work's success conditions and treat *usually* as non-normative; require Activities and Sub-processes describing the whole work to cover every Outcome and contribute to the Purpose, and those describing only selected work to identify the Outcomes they support; restate subject-less instructions as explicit requirements or recommendations, including scope clarification, element classification, shared-information relationships, reproducibility identifiers, unconfirmed references, change types, evaluation questions, and note labels in other languages; require View proposals to be distinguished from source requirements and exclusions to have a rationale; prohibit silently converting unconfirmed facts, assumptions, unmet conditions, or exclusions into success; and use *can* for possibility. Japanese descriptive statements no longer read as instructions.
- Revise Process Description Design: state its boundary with configuration design in the Purpose and description; make each Outcome a result condition that create, revise, and review requests can meet or qualify with findings, with change identification and selection justification as separate Outcomes, and allow findings to qualify an Outcome in creation or revision only for conditions unattainable within the request, information, or authority; require reading the Framework in full, limit judgments that depend on unread references, asking the requester about unresolvable gaps, Framework-aligned coverage and change types, and re-evaluation when bases change; move request scope to Constraints. In the examples, give Production Release two Outcomes with the health endpoint as an acceptance criterion, attribute the shared-brief requirements to a fictional working agreement, and use *responsibility*; in the template, use *result condition* and place the translation link after the title.
...
- Make Agent Work System Design independent of the other Skill and `examples/`: it starts from a description of the work in any form, states that defining what the work must achieve is outside its scope, applies only the design principles and Agent Skills sources, and uses self-contained design examples. Its Outcomes now separately cover respect for applicable requirements, authority, and effect limits, and allow review findings to qualify each result; Tasks add allocation criteria, least access and authority, untrusted information handling, and reproducibility, and use exposed-operation terminology. Repository guidance, development Skills, contribution guidance, the pull request template, and the working example's evaluation cases assess requirement respect separately from tool behavior and effectiveness, and the distribution test no longer allows links outside a Skill.
...
- Bundle the Process Framework and work-system design principles in their corresponding Skills' references, and place Agent Skill application rules in the design Skills.
- Define Markdown Process section and explanatory-note rules in the Framework, using `NOTE` in English and `注記` in Japanese. Apply the writing structure to both design Skills, and clarify how tools implement Controls and enforce Constraints alongside their enabling role.
...
- The separate ALPS Specification and `spec/` directory; the design Skills and their referenced principles carry the applicable rules.
