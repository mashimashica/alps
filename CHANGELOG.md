...
- Revise Process Description Design: state its boundary with configuration design in the Purpose and description; make each Outcome a result condition that create, revise, and review requests can meet or qualify with findings, including change and selection justification; require reading the Framework in full, asking the requester about unresolvable gaps, Framework-aligned coverage and change types, and re-evaluation when bases change; move request scope to Constraints. In the examples, give Production Release two Outcomes with the health endpoint as an acceptance criterion, mark the shared-brief requirements as fictional, and use *responsibility*; in the template, use *result condition* and place the translation link after the title.
