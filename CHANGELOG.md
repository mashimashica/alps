...
- Tighten the service-change example tool: reject request identifiers with surrounding whitespace, non-decimal numbers, and UTF-8 BOM input; report `input_relationship` (identical bytes and same request identifiers) and require matching identifiers for a positive pilot assessment; align fixture identifiers; state Python 3.10 or later and test it in CI; and move the example's Framework reference from Controls to Resources.
