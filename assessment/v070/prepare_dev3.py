"""Prepare the six final-development creators without consumer evidence."""
...
CELLS = (("S10", "A", 1), ("S10", "B3", 1),
         ("S05", "B3", 1), ("S05", "A", 1),
         ("S10", "B3", 2), ("S10", "A", 2))


def main():
    ledger = ROOT / "dev3-list.tsv"
    if ledger.exists():
        raise SystemExit("H03 assignments already exist; preserve them")
    for document in ("development-round-3.md", "development-2-closeout.md",
                     "audits/dev2-progression-review.md", "audits/candidate-b3-review.md",
                     "candidate-B3-hashes.json"):
        if not (ROOT / document).is_file():
            raise SystemExit(f"Reviewed basis not yet present: {document}")
    # Coordinator must read the review; existence never grants semantic approval.
    candidate = ROOT / "frozen/candidate-B3"
    frozen_hashes = json.loads((ROOT / "candidate-B3-hashes.json").read_text())
    actual = {p.relative_to(candidate).as_posix() for p in candidate.rglob("*")
              if p.is_file() or p.is_symlink()}
    if actual != set(frozen_hashes):
        raise SystemExit("Frozen candidate file set changed")
...
Work only on this task. Read only your input folder, the supplied authoring resources and required official Agent Skills format sources. Use {trial} as the working directory for task-local commands; do not enumerate the shared workspace. Do not inspect sibling trials, evaluation plans, other worktrees, audit reports, expected results, GitHub PRs/history or unrelated resources. Do not delegate. This is one fresh authoring task; shared filesystem access does not authorize reading other tasks.
...
"""
        ...
        assignment = {"trial_id": trial_id, "stage": "development-3", "case": case,
    ...
    print("Prepared six H03 creators; no consumer evidence copied")
