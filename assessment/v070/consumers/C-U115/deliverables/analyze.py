...
), True, "0 <= errors <= mature <= shipped; 0 <= catches <= shipped"))
...
), True, "overtime and training do not exceed productive hours"))
...
for period in ("baseline", "pilot"):
    for line in ("Harbor", "Ridge"):
        ...
        labor[(period, line)] = {
            "shifts": len(rows), "productive": productive, "training": training,
...
print()
print("| Period | Line | Full shifts | Productive h | Training h | Recurring h | Recurring h/shift | h/order |")
print("|---|---|---:|---:|---:|---:|---:|---:|")
for period in ("baseline", "pilot"):
    for line in ("Harbor", "Ridge"):
        x = labor[(period, line)]
        print(f"| {period} | {line} | {int(x['shifts'])} | {x['productive']:.0f} | {x['training']:.0f} | {x['recurring']:.0f} | {x['avg']:.2f} | {x['hours_per_order']:.5f} |")
print()
