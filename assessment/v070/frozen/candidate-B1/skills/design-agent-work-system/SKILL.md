...
4. Confirm required conditions before dependent operations. Address uncertain or partial effects before retrying an operation that changes state.
    ...
    > NOTE Confirming a condition and performing an operation that establishes it are different. Existing applicable evidence can already support a condition. Refreshing that evidence can change the basis on which another condition, such as an approval, depends.
