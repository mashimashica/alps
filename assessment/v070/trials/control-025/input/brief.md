...
The requested month and all supplied purchase-order lines identify the review scope. Coverage declarations concern those lines for that month; they do not remove a supplied line from scope. The coordinator checks the scope and source data, calculates the signed received quantity, interprets the receipt position, and directs each necessary follow-up using the supplied responsibilities. Returning a calculation without deciding the follow-up does not finish this work.
...
For a line with confirmed complete and valid evidence, compare net received with ordered quantity: equal means received as ordered; below means the remaining quantity needs follow-up; above means the excess quantity needs reconciliation. A partial export permits stating an observed subtotal, but not claiming the final receipt position. Do not replace missing values with zero. If a current-month event identifies an in-scope order but a SKU absent from that order, the affected order's receiving evidence needs identity reconciliation before final line comparisons. An event for an order outside the supplied order set is outside this review and does not change in-scope totals. If the same event_id has different content, identify the affected lines as conflicting evidence. Exact repeated copies of the same event count once.
