...
- H02 is preregistered in development-round-2.md. B2 is frozen with hashes/patch; independent semantic/locale review found no blocker and exact hashes match. Root nine product tests, whitespace and both Skill form checks pass. Eight creators and all sixteen valid consumer applications are complete. D2-U012 and D2-U014 originals were interrupted by recorded agent loss; their independently adjudicated first retries are complete and selected. Source-fidelity review is read; S05/S10 primary/second blind grades are running. These reuse developmental fixtures, not new holdout families. Latest resource check: 58 prepared assignments, zero unexpected changes to recorded files. No source or behavioral improvement is accepted yet.
- Experiment-only paired calculation helper and 13 meaningful numeric/denominator tests are implemented. Independent review reproduced an exact-20-percent roundoff bug and identified missing case-type grouping and frozen-schedule verification. Fixes are written; independent follow-up is running. The immutable schedule and whole-application case types must be saved before their respective execution/evaluation, with verified digests used for final analysis. No actual main observations exist and no numerical gate has been evaluated on task results.
...
Additional diagnostic reserve (separate from base): at most 72 executions across at most three rounds. H01's 24 and H02's eight creators plus sixteen valid consumers are completed (48 executions). Two interrupted original attempts remain separately retained, for 50 executions started in total and 22 reserve slots unallocated. Completion, observation validity, business adequacy and Outcome achievement are distinct.
